# -*- coding: utf-8 -*-
"""
Created on Fri Nov  5 14:07:30 2021
...
def get_options(current_date, current_time, ticker, opt_type, price_type, r):
    ...
    # Get exps plus additional feature:
    exps_1 = ticker.options
    for i in range(len(exps_1)):
        print(f'press {i} to select {exps_1[i]}')

    print(f'\nSelect a date between {exps_1[0]} and {exps_1[-1]} using numbers between 0 and {len(exps_1)-1}')
    exp_string_1 = 'Select the number on the left side of the exp date for the lower range value: >> '
    exp_string_2 = 'Select the upper range of the date period: >> '
     
    exp_input = input(exp_string_1)
    exp_input_2 = input(exp_string_2)
    ...
    while stop_loop == 0:
        ...
        try:
            exp_input, exp_input_2 = int(exp_input), int(exp_input_2)+1
            exps = exps_1[exp_input:exp_input_2]
            exp_input_2 = exp_input_2-1
            exp_min = exps_1[exp_input_2]
            exp_max = exps_1[exp_input]
            
            stop_loop = 1
            
        except:
            print(f'\nERROR; Please select a number between 0 and {len(exps_1)-1} corresponding to dates between {exps_1[0]} and {exps_1[-1]}\n')
            exp_input = input(exp_string_1)
            exp_input_2 = input(exp_string_2)
    
    # The closer of the expiration date the cheaper the value
    
    op_ch = ticker.option_chain(exp_min)
    min_calls = min(op_ch.calls['strike'])
    min_puts = min(op_ch.puts['strike'])
    
    op_ch = ticker.option_chain(exp_max)
    max_calls = max(op_ch.calls['strike'])
    max_puts = max(op_ch.puts['strike'])
    
    # filter calls by strike price
    if opt_type in ['calls', 'both']:
        price_calls_strike = []
        print(f'\n Select a strike price for calls from {min_calls} to {max_calls}')
        
        strike_string = 'Select the lower strike price calls: >> '
        strike_string_2 = 'Select the upper strike price calls: >> '
        
        strike_input = input(strike_string)
        strike_input_2 = input(strike_string_2)
        
        stop_loop = 0
        
        while stop_loop == 0:
            try:
                strike_input, strike_input_2 = float(strike_input), float(strike_input_2)
    ...
    if opt_type in ['puts','both']:
        #filter puts by strike price

        price_puts_strike = []
        print(f'Select a strike price for puts from {min_puts} to {max_puts}')
        
        string_input_put = 'Select the lower strike price put: >> '
        string_input_put_2 = 'Select the upper strike price put: >> '
        
        
        strike_input_put = input(string_input_put)
        strike_input_put_2 = input(string_input_put_2)
        stop_loop = 0
        
        while stop_loop == 0:
            try:
                strike_input_put, strike_input_put_2 = float(strike_input_put), float(strike_input_put_2)
                stop_loop = 1
    ...
    for exp in exps:
        ...
        strike_price_calls = calls['strike']
        # Filter the options by strike price
        
        if opt_type in ['calls','both']:
            calls = calls[calls['strike'].between(strike_input, strike_input_2)]
        ...
        strike_price_puts = puts['strike']
        
        if opt_type in ['puts', 'both']:
            puts = puts[puts['strike'].between(strike_input_put, strike_input_put_2)]
        ...
        if opt_type in ['calls','both']:
            ...
            for key in call_objects.keys():
                for i in range(len(call_objects[key])):
                    df_call.loc[i, ['IV']] = call_objects[key][i].IV
                    df_call.loc[i, ['K']] = call_objects[key][i].K
                    df_call.loc[i, ['Lambda']] = call_objects[key][i].Lambda
                    df_call.loc[i, ['S']] = call_objects[key][i].S
                    df_call.loc[i, ['V']] = call_objects[key][i].V
                    df_call.loc[i, ['ask']] = call_objects[key][i].ask
                    df_call.loc[i, ['bid']] = call_objects[key][i].bid
                    df_call.loc[i, ['charm']] = call_objects[key][i].charm
                    df_call.loc[i, ['color']] = call_objects[key][i].color
                    df_call.loc[i, ['delta']] = call_objects[key][i].delta
                    df_call.loc[i, ['exp']] = call_objects[key][i].exp
                    df_call.loc[i, ['gamma']] = call_objects[key][i].gamma
                    df_call.loc[i, ['itm']] = call_objects[key][i].itm
                    df_call.loc[i, ['openInterest']] = call_objects[key][i].openInterest
                    df_call.loc[i, ['opt_type']] = call_objects[key][i].opt_type
                    df_call.loc[i, ['q']] = call_objects[key][i].q
                    df_call.loc[i, ['r']] = call_objects[key][i].r
                    df_call.loc[i, ['rho']] = call_objects[key][i].rho
                    df_call.loc[i, ['speed']] = call_objects[key][i].speed
                    df_call.loc[i, ['t']] = call_objects[key][i].t
                    df_call.loc[i, ['theta']] = call_objects[key][i].theta
                    df_call.loc[i, ['ultima']] = call_objects[key][i].ultima
                    df_call.loc[i, ['vanna']] = call_objects[key][i].vanna
                    df_call.loc[i, ['vega']] = call_objects[key][i].vega
                    df_call.loc[i, ['veta']] = call_objects[key][i].veta
                    df_call.loc[i, ['volume']] = call_objects[key][i].volume
                    df_call.loc[i, ['vomma']] = call_objects[key][i].zomma
        ...
        if opt_type in ['puts','both']:
            ...
            for key in put_objects.keys():
                for i in range(len(put_objects[key])):
                    df_put.loc[i, ['IV']] = put_objects[key][i].IV
                    df_put.loc[i, ['K']] = put_objects[key][i].K
                    df_put.loc[i, ['Lambda']] = put_objects[key][i].Lambda
                    df_put.loc[i, ['S']] = put_objects[key][i].S
                    df_put.loc[i, ['V']] = put_objects[key][i].V
                    df_put.loc[i, ['ask']] = put_objects[key][i].ask
                    df_put.loc[i, ['bid']] = put_objects[key][i].bid
                    df_put.loc[i, ['charm']] = put_objects[key][i].charm
                    df_put.loc[i, ['color']] = put_objects[key][i].color
                    df_put.loc[i, ['delta']] = put_objects[key][i].delta
                    df_put.loc[i, ['exp']] = put_objects[key][i].exp
                    df_put.loc[i, ['gamma']] = put_objects[key][i].gamma
                    df_put.loc[i, ['itm']] = put_objects[key][i].itm
                    df_put.loc[i, ['openInterest']] = put_objects[key][i].openInterest
                    df_put.loc[i, ['opt_type']] = put_objects[key][i].opt_type
                    df_put.loc[i, ['q']] = put_objects[key][i].q
                    df_put.loc[i, ['r']] = put_objects[key][i].r
                    df_put.loc[i, ['rho']] = put_objects[key][i].rho
                    df_put.loc[i, ['speed']] = put_objects[key][i].speed
                    df_put.loc[i, ['t']] = put_objects[key][i].t
                    df_put.loc[i, ['theta']] = put_objects[key][i].theta
                    df_put.loc[i, ['ultima']] = put_objects[key][i].ultima
                    df_put.loc[i, ['vanna']] = put_objects[key][i].vanna
                    df_put.loc[i, ['vega']] = put_objects[key][i].vega
                    df_put.loc[i, ['veta']] = put_objects[key][i].veta
                    df_put.loc[i, ['volume']] = put_objects[key][i].volume
                    df_put.loc[i, ['vomma']] = put_objects[key][i].zomma
    ...
    while stop_loop == 0:
        try:
            ...
            if opt_type == 'calls':
                df_call.to_csv('calls.csv', index = False)
                return {'calls': call_objects}
         
            elif opt_type == 'puts':
                df_put.to_csv('puts.csv', index = False)
            ...
            if error_print:
                print("\nERROR, Please close your csv file to continue.........\n")
            error_print = False
...
def main():
    ...
    # Print the read me
    print('\n---------------------------------------READ ME---------------------------------------------\n')
    print('| Data is pulled from Yahoo! finance with the yfinance python package. There is a rate    |')
    print('| limit of 2000 request per hour to the Yahoo! API, and each expiration in an option      |')
    print('| chain requires 1 API call. So if you are running this a lot, you may get odd behavior.  |')
 
    print('| Yahoo! also seems to "reset" around around 12-2am, so prices may all be 0 at that time. |')
 
    print('\n| Yahoo! does provide implied volatility numbers, but in the spirit of this exercise,     |')
    print('| IVs are manually calculated from the option price using a bisection algorithm.          |')
 
    print('\n| Bisection is slower than something like Newton-Raphson, but is guaranteed to converge   |')
    print('| given that the answer is between the initial guesses. The initial guesses for IV in     |')
    print('| this script are 0% and 2000%.                                                           |')
 
    print('\n| There are no filters with respect to open interest or volume. Whatever bid/ask/last is  |')
    print('| present will be used.                                                                   |')
 
    print('\n| For tickers with more than 13 expirations, only every other expiration will be labeled  |')
    print('| on the plots. Otherwise the labels get crowded and hard to read.                        |')
 
    print('\n------------------------------------------------------------------------------------------\n')
    input('Press any key to continue...\n')
 
    while stop_main_loop == '0':
 
        # Get mode
        mode = input(
            'Press 0 to make plots, 1 to calculate the standard greeks for a single option: ')
        ...
        if mode == '0':
            ...
            # Stop or continue
            stop_main_loop = input(
                'Enter 0 to continue, anything else to quit: ')
 
        # If single option mode
        elif mode == '1':
 
            # Get inputs
            current_date, current_time, opt_type, exp, V, S, K, r = single_option_input()
 
            # Create option object
            try:
                option = Option(current_date, current_time,
                                opt_type, exp, V, S, K, r, 0, 0, 0, 0)
 
                # Names of things to print
                names = [
                    'IV',
                    'delta',
                    'gamma',
